...
function makeRequest() {
	request('GET', `https://reqres.in/api/users?page=${pageNumber.value}&per_page=12`)
		.then(function (response) {
			const cardsArray = response.data;
...
function makeRequest(page) {
	request('GET', `https://reqres.in/api/users?page=${page}&per_page=${notesOnPage}`)
		.then(function (response) {
...
.then(function (response){
	totalPages = Math.ceil(response.total / notesOnPage)
})
...
function createPagination(totalPages, page) {
	...
	if (page > 1) { //show the next button if the page value is greater than 1
		liTag += `<li class="btn prev" onclick="createPagination(totalPages, ${page - 1})"><span><i class="fa fa-arrow-circle-o-left" aria-hidden="true"></i></span></li>`;
	}
	...
	if (page < totalPages) { //show the next button if the page value is less than totalPage(20)
		liTag += `<li class="btn next" onclick="createPagination(totalPages, ${page + 1})"><span><i class="fa fa-arrow-circle-o-right" aria-hidden="true"></i></span></li>`;
	}
